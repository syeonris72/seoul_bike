...
| :--- |:---------------------------------------------------------------------------|:---------------------------------------------------------------------------------------|
| **장수연** | 베이스라인 전체 통합 및 LightGBM 하이퍼파라미터 튜닝, Supabase-S3 아키텍처 연동 | LightGBM 트리 구조/학습률 최적화로 단일 모델 최고 성능 달성 |
| **박은비** | 베이스라인 지원 및 XGBoost 하이퍼파라미터 튜닝                           | XGBoost 최적 파라미터 탐색으로 Baseline 대비 오차 개선                                                 |
| **김세호** | 베이스라인 지원 및 선형 회귀 계열 모델 하이퍼파라미터 튜닝                           | 선형 모델의 한계 분석                                                                           |
| **권덕윤** | 베이스라인 지원 및 RandomForest 하이퍼파라미터 튜닝                            | `max_depth` 및 샘플 분할 조건 최적화                                                             |
...
- 관리자 관제 지도의 실시간 재고 수집 스케줄러(APScheduler)가 주기적으로 값을 덮어쓰면서, 그 사이 발생한 대여/반납 트랜잭션 결과가 유실되는 재고 불일치 현상 발생 → 스케줄러는 기상·인구 등 외부 피처 갱신만 전담하도록 역할을 분리하고, 대여소 재고(`station_stock`)는 오직 대여·반납 트랜잭션(`rental` 상태 변경)에서만 갱신되도록 구조를 재설계해 실시간 데이터 정합성 확보
- 팀원별로 로컬 환경(패키지 버전, 모델 아티팩트 경로 등)이 달라 병합 시마다 코드가 깨지는 문제 반복 → Supabase Storage에 `best_params.json`과 모델 아티팩트를 단일 소스로 저장하고 서버/파이프라인 구동 시 `boto3`로 동적 Fetch하도록 통일해 팀원 간 환경 불일치 및 휴먼 에러 원천 차단
- Optuna 하이퍼파라미터 튜닝 시 전체 변수 공간을 한 번에 탐색하려다 trial당 소요 시간이 지나치게 길어짐 → 모델별로 탐색 전략을 3~4개로 나누고 trial 수를 30~80회로 제한한 다단계 탐색 방식으로 전환해 제한된 시간 안에서도 유의미한 성능 개선 확보

### 프로젝트 회고

> 장수연(PM)
- 데이터 분석 및 모델링 과정에서 팀원 지도 및 공백 인원의 역할까지 병행하며 프로젝트 총괄 완수 
- 관리자 페이지의 실시간 데이터 연동 과정에서 발생한 예상치 못한 난제를 끝까지 파고들어 해결하며 핵심 기능 구현 성공 
- 파편화된 팀원들의 미완성 작업 및 코드를 직접 수정 및 병합하여 단일 파이프라인으로 통합하고, GitHub 및 Notion 기반의 프로젝트 전담 문서화 완수 
- 방대한 데이터와 과도한 목표 설정에 따른 일정 지연 및 인력 관리의 어려움 ⇒ PM으로서의 막중한 책임감과 체계적인 리소스 관리의 중요성을 깨닫고 실무 리더십 체득 
- 이번 프로젝트에서 축소되었던 분석 범위(자치구) 확대, 다인용 이용권 지원, 하이퍼파라미터 튜닝 실험 다양화 등 향후 서비스의 기능적 확장 및 성능 고도화 다짐

> 박은비(Deputy PM)
- SHP 포맷 학습을 통해 면적 형태의 공간 데이터 수집 및 가공 성공
- 날씨/인구/대여 기록 등 서로 다른 시간 기준의 시계열 데이터를 통합하며 데이터 품질의 중요성 체감
- 데이터 가공부터 AI 모델 구동까지의 전 과정 경험 및 주요 변수 영향력에 대한 직관 확보
- 일반 웹 기능 구현으로 인해 관리자용 ｢데이터 분석｣ 대시보드 고도화 미흡
⇒ 향후 프로젝트 시 부가 기능보다 서비스 핵심 가치 구현에 리소스를 최우선으로 투입하는 기준 확립

> 전지혜
- 파생변수 생성 및 앙상블 기법을 적용하여 모델 성능 향상, RMSLE 지표 기반의 객관적인 성과 개선 시각화 구현 
- 프로젝트 전체 흐름 파악의 중요성 체감, 메모리 관리 등 데이터 엔지니어링 기반이 시스템 안정성을 좌우한다는 실무적 시각 체득 
- 시각화 자료의 정보 전달력을 보완하기 위해 분석 역량을 지속 강화 필요
- 대용량 데이터 처리 및 환경 최적화 등 데이터 파이프라인 구축 역량 고도화 다짐

> 김세호
- 데이터 파이프라인(수집/정제/DB 연동) 작업의 난이도 과소평가 및 잠재 변수 대비 부족으로 초기 슬럼프 경험
- 팀에 대한 책임감으로 위기 극복, 타인 의존 없이 공식 문서를 활용한 주도적인 디버깅으로 기능 구현 완수
- 데이터 처리 단계의 세밀한 분할(수집-정제-적재)과 잠재 오류 사전 학습을 통해 심리적 압박을 줄이고 체계적인 디버깅 환경 구축

> 권덕윤
- 다범주 변수(대여소 ID 등)에 타겟 인코딩을 적용해 AI 예측 성능을 높이고, 모델-API 연동 구현 지원 성공
- 잦은 UI 요구사항 변경에 맞춰 복잡한 다계층 계정 권한(RBAC) 로직을 동시에 수정 및 동기화 작업 완수
- 풀스택-AI 간의 유기적인 시스템 연관성을 체득하고, 역할별 데이터 접근 권한(Data Visibility)을 고려해 화면을 설계하는 실무 시각 체득
- 타 파트와의 연동 지점(Interface) 및 코드 의존성을 선제적으로 파악하여, 예기치 않은 시스템 충돌을 막고 협업 효율 극대화 다짐
